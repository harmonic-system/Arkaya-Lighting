...
const AdminStageLightingSystems = () => {

    const { token, server } = useAuthContext()
    const { stageLightingSystem, getAllStageLightingSystemProducts } = useRigging()
    ...
    const deleteProduct = async (id) => {
        try {
            const response = await fetch(`${server}/api/v1/rigging/adminstageLightingSystems/deletestagelightingsystem/${id}`, {
                method: "DELETE",
                headers: {
                    'Authorization': token,
                }
            ...
            if (response.ok) {
                ...
                getAllStageLightingSystemProducts()
            }
        } catch (error) {
    ...
    return (
        <>
            <div className="container my-5">
                <h2 className="fw-bold mb-3">Admin Stage Lighting Systems Products</h2>
                <div className="w-100 d-flex justify-content-end my-5">
                    <Link className="btn btn-warning" to="/admin/addstagelightingsystem" >Add Product</Link>
                </div>
                <table className="table">
                    ...
                    <tbody>
                        {
                            stageLightingSystem.map((product) => {
                                ...
                                return <tr key={product._id}>
                                    ...
                                    <td><Link to={`/admin/editstagelightingsystem/${product._id}`} className="bg-transparent" ><FaEdit className="bg-transparent" /></Link></td>
...
export default AdminStageLightingSystems;