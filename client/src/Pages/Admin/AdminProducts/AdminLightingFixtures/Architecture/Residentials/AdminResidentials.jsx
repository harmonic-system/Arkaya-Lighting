...
const AdminResidentials = () => {

    const { authorizationToken, server } = useAuth()
    const { residential, getAllResidentialProducts } = useLightingFixtures()
    ...
    const deleteProduct = async (id) => {
        try {
            const response = await fetch(`${server}/api/v1/lightingfixture/adminarchitecture/deleteresidential/${id}`, {
                method: "DELETE",
                headers: {
    ...
    return (
        <>
            <div className="container my-5">
                <h2 className="fw-bold mb-3">Admin Residential Products</h2>
                <div className="w-100 d-flex justify-content-end my-5">
                    <Link className="btn btn-warning" to="/admin/addresidential" >Add Product</Link>
                </div>
                    ...
                    <tbody>
                        {
                            residential.map((product) => {
                                ...
                                return <tr key={product._id}>
                                    ...
                                    <td><Link to={`/admin/editresidential/${product._id}`} className="bg-transparent" ><FaEdit className="bg-transparent" /></Link></td>
...
export default AdminResidentials;