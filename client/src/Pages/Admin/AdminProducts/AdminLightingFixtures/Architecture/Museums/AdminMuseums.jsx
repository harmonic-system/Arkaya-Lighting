...
const AdminMuseums = () => {
    ...
    const { museum, getAllMuseumProducts } = useLightingFixtures()
    ...
    const deleteProduct = async (id) => {
        try {
            const response = await fetch(`${server}/api/v1/lightingfixture/adminarchitecture/deletemuseum/${id}`, {
                method: "DELETE",
                headers: {
    ...
    return (
        <>
            <div className="container my-5">
                <h2 className="fw-bold mb-3">Admin Museum Products</h2>
                <div className="w-100 d-flex justify-content-end my-5">
                    <Link className="btn btn-warning" to="/admin/addmuseum" >Add Product</Link>
                </div>
                    ...
                    <tbody>
                        {
                            museum.map((product) => {
                                ...
                                return <tr key={product._id}>
                                    ...
                                    <td><Link to={`/admin/editmuseum/${product._id}`} className="bg-transparent" ><FaEdit className="bg-transparent" /></Link></td>
