...
const EditAdminMediaPixel = () => {
  ...
  const getSingleProduct = async (id) => {
    const response = await fetch(`${server}/api/v1/lightingfixture/adminarchitecture/getsingalmediapixel/${id}`, {
      method: "GET",
      headers: {
        "Authorization": token
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${server}/api/v1/lightingfixture/adminarchitecture/updatemediapixel/${id}`, {
        method: "PUT",
        headers: {
          "Authorization": token,
...
export default EditAdminMediaPixel;