...
const AdminTelevision = () => {

    const { token, server } = useAuthContext()
    const { television, getAllTelevisionProducts } = useLightingFixtures()
    ...
    const deleteProduct = async (id) => {
        try {
            const response = await fetch(`${server}/api/v1/lightingfixture/adminthreaterstudiotelevision/deletetelevision/${id}`, {
                method: "DELETE",
                headers: {
                    'Authorization': token,
                }
    ...
    return (
        <>
            <div className="container my-5">
                <h2 className="fw-bold mb-3">Admin Television Products</h2>
                <div className="w-100 d-flex justify-content-end my-5">
                    <Link className="btn btn-warning" to="/admin/addtelevision" >Add Product</Link>
                </div>
                    ...
                    <tbody>
                        {
                            television.map((product) => {
                                ...
                                return <tr key={product._id}>
                                    ...
                                    <td><Link to={`/admin/edittelevision/${product._id}`} className="bg-transparent" ><FaEdit className="bg-transparent" /></Link></td>
...
export default AdminTelevision;