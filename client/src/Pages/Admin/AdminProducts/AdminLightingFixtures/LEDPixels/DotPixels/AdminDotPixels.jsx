...
const AdminDotPixels = () => {

    const { authorizationToken, server } = useAuth()
    const { dotPixel, getAllDotPixelProducts } = useLightingFixtures()
    ...
    const deleteProduct = async (id) => {
        try {
            const response = await fetch(`${server}/api/v1/lightingfixture/adminledpixels/deletedotpixel/${id}`, {
                method: "DELETE",
                headers: {
    ...
    return (
        <>
            <div className="container my-5">
                <h2 className="fw-bold mb-3">Admin Dot Pixels Products</h2>
                <div className="w-100 d-flex justify-content-end my-5">
                    <Link className="btn btn-warning" to="/admin/adddotpixel" >Add Product</Link>
                </div>
                    ...
                    <tbody>
                        {
                            dotPixel.map((product) => {
                                ...
                                return <tr key={product._id}>
                                    ...
                                    <td><Link to={`/admin/editdotpixel/${product._id}`} className="bg-transparent" ><FaEdit className="bg-transparent" /></Link></td>
