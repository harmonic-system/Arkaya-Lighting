...
const AdminLedMatrix = () => {

    const { token, server } = useAuthContext()
    const { ledMatrix, getAllLedMatrixProducts } = useLightingFixtures()
    ...
    const deleteProduct = async (id) => {
        try {
            const response = await fetch(`${server}/api/v1/lightingfixture/adminledpixels/deleteledmatrix/${id}`, {
                method: "DELETE",
                headers: {
                    'Authorization': token,
                }
    ...
    return (
        <>
            <div className="container my-5">
                <h2 className="fw-bold mb-3">Admin Led Matrix Products</h2>
                <div className="w-100 d-flex justify-content-end my-5">
                    <Link className="btn btn-warning" to="/admin/addledmatrix" >Add Product</Link>
                </div>
                    ...
                    <tbody>
                        {
                            ledMatrix.map((product) => {
                                ...
                                return <tr key={product._id}>
                                    ...
                                    <td><Link to={`/admin/editledmatrix/${product._id}`} className="bg-transparent" ><FaEdit className="bg-transparent" /></Link></td>
