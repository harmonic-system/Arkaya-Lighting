...
const AdminIndoorSeries = () => {

    const { authorizationToken, server } = useAuth()
    const { indoorSeries, getAllIndoorSeriesProducts } = useVideoDisplays()
    ...
    const deleteProduct = async (id) => {
        try {
            const response = await fetch(`${server}/api/v1/videodisplays/adminindoorseries/deleteindoorseries/${id}`, {
                method: "DELETE",
                headers: {
    ...
    return (
        <>
            <div className="container my-5">
                <h2 className="fw-bold mb-3">Admin Indoor Series Products</h2>
                <div className="w-100 d-flex justify-content-end my-5">
                    <Link className="btn btn-warning" to="/admin/addindoorseries" >Add Product</Link>
                </div>
                    ...
                    <tbody>
                        {
                            indoorSeries.map((product) => {
                                ...
                                return <tr key={product._id}>
                                    ...
                                    <td><Link to={`/admin/editindoorseries/${product._id}`} className="bg-transparent" ><FaEdit className="bg-transparent" /></Link></td>
...
export default AdminIndoorSeries;