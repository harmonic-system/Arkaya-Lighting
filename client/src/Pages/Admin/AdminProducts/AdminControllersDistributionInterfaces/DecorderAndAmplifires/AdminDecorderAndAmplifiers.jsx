...
const AdminDecorderAndAmplifiers = () => {

    const { authorizationToken, server } = useAuth()
    const { decorderAndAmplifiers, getAllDecorderAndAmplifiersProducts } = useCDI()
    ...
    const deleteProduct = async (id) => {
        try {
            const response = await fetch(`${server}/api/v1/controllerdistributioninterfaces/adminDecorderAndAmplifiers/deletesignaldistribution/${id}`, {
                method: "DELETE",
                headers: {
            ...
            if (response.ok) {
                ...
                getAllDecorderAndAmplifiersProducts()
            }
        } catch (error) {
    ...
    return (
        <>
            <div className="container my-5">
                <h2 className="fw-bold mb-3">Admin Decorder And Amplifiers Products</h2>
                <div className="w-100 d-flex justify-content-end my-5">
                    <Link className="btn btn-warning" to="/admin/adddecorderandamplifiers" >Add Product</Link>
                </div>
                <table className="table">
                    ...
                    <tbody>
                        {
                            decorderAndAmplifiers.map((product) => {
                                ...
                                return <tr key={product._id}>
                                    ...
                                    <td><Link to={`/admin/editdecorderandamplifiers/${product._id}`} className="bg-transparent" ><FaEdit className="bg-transparent" /></Link></td>
...
export default AdminDecorderAndAmplifiers;