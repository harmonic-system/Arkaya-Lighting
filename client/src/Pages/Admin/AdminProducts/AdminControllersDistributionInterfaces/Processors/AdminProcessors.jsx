...
const AdminProcessoers = () => {

    const { authorizationToken, server } = useAuth()
    const { processors, getAllProcessorsProducts } = useCDI()
    ...
    const deleteProduct = async (id) => {
        try {
            const response = await fetch(`${server}/api/v1/controllerdistributioninterfaces/adminProcessoers/deleteprocessor/${id}`, {
                method: "DELETE",
                headers: {
    ...
    return (
        <>
            <div className="container my-5">
                <h2 className="fw-bold mb-3">Admin Processors Products</h2>
                <div className="w-100 d-flex justify-content-end my-5">
                    <Link className="btn btn-warning" to="/admin/addprocessor" >Add Product</Link>
                </div>
                    ...
                    <tbody>
                        {
                            processors.map((product) => {
                                ...
                                return <tr key={product._id}>
                                    ...
                                    <td><Link to={`/admin/editprocessor/${product._id}`} className="bg-transparent" ><FaEdit className="bg-transparent" /></Link></td>
...
export default AdminProcessoers;